...
### ARIMA MODELS
egizio_ts=ts(egizio_train_df$visitors, frequency = 12)
arima<-auto.arima(egizio_ts)
predicted<-forecast(arima, h=12)
mse <- mean((egizio_test_df$visitors- predicted$mean)^2)
mse
summary(arima)
start_date<-as.Date("2022-01-01")
# union of real nd predicted data
grafico_dati <- data.frame(
  Date = seq(from = start_date, by = "month", length.out = 12),
  Forecast = predicted$mean,
  Visitors = egizio_test_df$visitors
)

# Crea il grafico
ggplot(grafico_dati, aes(x = Date)) +
  geom_line(aes(y = Visitors, color = "Actual Values"), size = 1.2) +
  geom_line(aes(y = Forecast, color = "Predicted Values"), linetype = "dashed", size = 1) +
  labs(title = "Confronto tra Previsioni e Valori Effettivi", x = "Data", y = "Valori") +
  scale_color_manual(values = c("Actual Values" = "blue", "Predicted Values" = "red"))

# Calculate metrics for ARIMA
r_squared <- 1 - mean((egizio_test_df$visitors-predicted$mean) ^ 2) / mean((egizio_test_df$visitors - mean(egizio_test_df$visitors)) ^ 2)
mse <- mean((egizio_test_df$visitors -predicted$mean) ^ 2)
rmse <- sqrt(mean((egizio_test_df$visitors - predicted$mean) ^ 2))
mae <- mean(abs(egizio_test_df$visitors - predicted$mean))
mape <- mean(abs((egizio_test_df$visitors - predicted$mean) / egizio_test_df$visitors)) * 100
n <- nrow(egizio_test_df)
sse <- sum((egizio_test_df$visitors - predicted$mean) ^ 2)
k <- 1
aic <- n * log(sse / n) + 2 * k

# Update metrics_df with ARIMA metrics
metrics_df <- rbind(metrics_df, list(Model = "ARIMA",
                                     R2 = NA, MSE = mse,
                                     RMSE = rmse, MAE = mae,
                                     MAPE = mape, AIC = aic)) 
...
# order to see if there are again some important behaviours
...
# So we need to model this particular characteristics.

# We try now to build first Arima models
arima = Arima(egizio_ts, order = c(1,1,2), seasonal = c(1,1,2))
arima
#order = c(0,1,2), seasonal = c(0,1,2)
# Arima (0,1,0) (0,0,2) : AIC=7090.42 
# Arima (2,1,0) (0,0,2) : AIC=6971.78
# Arima (0,0,2) (0,1,2) :
# --> with this model it's clear that there's a pattern that
#     the model is nor so able to understand
# Arima (0,1,2) (0,1,2) : AIC=6645.95

grafico <- ggplot(data = data.frame(egizio_ts), aes(x = time(egizio_ts), y = egizio_ts)) +
  geom_line(color = "blue") +  
  geom_line(aes(y = fitted(arima)), color = "red", linetype = "twodash") +  # Linea dei valori adattati dal modello ARIMA
  labs(title = "Confronto tra serie temporale originale e valori adattati dal modello ARIMA")
print(grafico)

p_arima_res = ggplot(aes(as.Date(date), y = residuals(arima)),
                     data =egizio_train_df)+
  geom_point(color = "blue")+
  xlab("Date")+
  ylab("Residuals")+
  ggtitle("Residuals of Arima")
plot(p_arima_res)
...
grafico_dati <- data.frame(
  Date = seq(from = start_date, by = "month", length.out = 12),
  Forecast = pred_arima_partial$mean,
  Visitors = egizio_test_df$visitors
)
...
# We see that for different windows of data the model needs a lot
# of training examples in order to fit well our series.
# So we try now to represent how the forecasting gets better 
# giving to the model a "cumulated information" through time:
...
# METTERE A POSTO FINESTRA PER I GRAFICI 

for(i in 1:N_WINDOWS){
  size = train_size * i
  train_w = egizio_ts[1:size]
  arima_w = Arima(train_w, order = c(0,1,2), 
                  seasonal = c(0,1,2))
  pred_arima_w = predict(arima_w, n.ahead = test_size)
  
  plot(x = (size):(size+test_size),egizio_ts[(size):(size+test_size)], type = "l", xlab = "Time",
       ylab = "CarSales", lwd = 1.5)
  lines(x = (size-12):(size-1), y = tail(arima_w$fitted,12), col = 2, lwd = 1.5)
  lines(x = (size):(size+test_size-1), y = tail(pred_arima_w$pred,test_size),
        col = 4, lwd = 1.5)
}
